...
class AppPermission {
  ...
  Future<void> permissionAccessGalleryOrCameraForCover(PermissionStatus status, BuildContext context, ImageSource source) async {
   ...
   Future<void> permissionAccessGalleryOrCameraForStory(
  ...
  Future<void> permissionAccessGalleryMultiplesImagesNewStory(PermissionStatus status, BuildContext context) async {
    ...
    switch (status){
      ...
      case PermissionStatus.granted:
          ...
          if( imagePath != null ){
            for (var image in imagePath) { 
              BlocProvider.of<StoryBloc>(context).add( OnSelectedImagePreviewEvent( File(image!.path) ));
            }
          }
        break;
      case PermissionStatus.denied:
  ...
  Future<void> permissionAccessGalleryOrCameraForNewTrip(PermissionStatus status, BuildContext context, ImageSource source) async {
    ...
    switch (status){
      ...
      case PermissionStatus.granted:
        final XFile? imagePath = await _picker.pickImage(source: source);
        if( imagePath != null ){
          BlocProvider.of<TripBloc>(context).add( OnSelectedImageEventTrip( File(imagePath.path) ));
        }
        break;
      case PermissionStatus.denied:
      case PermissionStatus.restricted:
  ...
  Future<void> permissionAccessGalleryMultiplesImagesNewTrip(PermissionStatus status, BuildContext context) async {
    ...
    switch (status){
      ...
      case PermissionStatus.granted:
        ...
        if( imagePath != null ){
          for (var image in imagePath) {
            BlocProvider.of<TripBloc>(context).add( OnSelectedImageEventTrip( File(image!.path) ));
          }
        }
        break;
      case PermissionStatus.denied:
